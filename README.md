...
- 将“大学副本地图”升级为更接近制霸路线图的“校园通关地图”视觉，5 个区域以整体地图节点和连线方式展示
- 每个区域新增项目小节点状态显示，区分未点亮、已探索、已制霸，更容易一眼看到进度
- 分享结果卡片新增压缩版 5 区域迷你地图，并高亮“最强副本”，更适合导出 PNG 后分享到小红书
- 保持 `ITEMS` 数量和顺序不变，继续兼容基于数组索引的 `localStorage` 存档
- 保留 `MAP_AREAS` 数据结构，仅增强地图呈现方式和同步更新逻辑
...
- 顶部结果区域适合截图分享
- 新增校园通关地图，展示 5 个区域的路线节点、完成率、状态、小型进度条和项目点亮情况
- 分享卡片内置迷你校园地图，展示 5 个区域完成率和最强副本高亮
- 支持复制分享文案
- 支持下载分享图 PNG
- 支持重置所有选择
